...
import LeaveRoomModal from './Blocks/LeaveRoomModal'
...
class Messages extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.socket.on('update_members', function (data) {
      let room_id = data.room_id;
      if (thisClass.state.activeRoom.id === room_id) {
        thisClass.setState({'activeRoom': {}});
        thisClass.selectRoom({'id': room_id});
      }
      let detach = _.isArray(data.detach) ? data.detach : [];
      if (detach.indexOf(thisClass.props.activeUser.id) !== -1) {
        thisClass.delRoomFromBar(room_id);
      }
    });

    this.props.socket.on('close_room', function (data) {
      let room_id = data.room_id;
      if (thisClass.state.activeRoom.id === room_id) {
  ...
  delRoomFromBar = (room_id) => {
    let rooms = this.state.rooms;
    rooms     = _.reject(rooms, (item) => {
  ...
  openModal      = (forUpdate = false) => {
    ...
    Api.User.list().then(response => {
      let selectedUsers = forUpdate ? _.map(this.state.activeRoom.members, (item) => {
        return {
          id: item.user.id,
  ...
  render() {
    return (
      <div className="container chat-container">
            ...
            <div className="dropdown-zone message-dropdown-zone">
              <Dropdown id="menuDropdown">
                ...
                <Dropdown.Menu className="super-colors">
                  ...
                  <MenuItem onClick={this.openLeaveModal.bind(this)} eventKey="2">Leave Room</MenuItem>
                </Dropdown.Menu>
              </Dropdown>
            </div>
            }
        ...
        <LeaveRoomModal
          thisClass={this}
          forClose={this.closeLeaveModal.bind(this)}
